...
if (isset($_POST['simpan'])) {
    $emailx = $_POST['emailx'];
    $cekEmail = $lib->get_by_email($emailx);
    if ($cekEmail < 1) {
        echo "<script type='text/javascript'>alert('Gagal Menambahkan, Email sudah digunakan!');window.location.href = 'ubah_data_karyawan?id=" . $data_karyawan['id_akses_login'] . "';</script>";
    } else {
        $idx_akses = $_POST['id_akses'];
        $nama_karyawanx = $_POST['namaKaryawan'];
        $jbtn = $_POST['jabatanx'];
        $alamatx = $_POST['alamatx'];
        $notlpx = $_POST['notlpx'];
        $gajix = $_POST['gajiKaryawan'];
        $status_gaji = $_POST['statusGaji'];
        $add_status = $lib->ubahKaryawan($idx_akses, $nama_karyawanx, $jbtn, $alamatx, $notlpx, $emailx, $gajix, $status_gaji);
        if ($add_status) {
            echo "<script type='text/javascript'>alert('Berhasil Mengubah Data Karyawan!');window.location.href = 'informasi_karyawan';</script>";
        } else {
            echo "<script type='text/javascript'>alert('Gagal Mengubah Data Karyawan!');window.location.href = 'ubah_data_karyawan?id=" . $data_karyawan['id_akses_login'] . "';</script>";
        }
    }
}
?>
...
<head>
    ...
    <title>SKVT - Ubah Data Karyawan</title>
                                            ...
                                            <div class="row">
                                                <div class="col-md-12 col-lg-12">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" id="nama_karyawan" value="<?= $data_karyawan['nama_karyawan']; ?>" required name="namaKaryawan" placeholder="Nama Karyawan">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="alamat">Alamat</label>
                                                        <textarea class="form-control" name="alamatx" required aria-label="With textarea" placeholder="Alamat" minlength="10"><?= $data_karyawan['alamat']; ?></textarea>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="notlp">No Telephone/WA</label>
                                                        <input type="text" class="form-control" id="notlp" value="<?= $data_karyawan['no_tlp']; ?>" required name="notlpx" placeholder="Nomor Telephone" minlength="10" onkeypress="return event.charCode >= 48 && event.charCode <=57">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="email">Email</label>
                                                        <input type="email" class="form-control text-lowercase" id="email" required value="<?= $data_karyawan['email']; ?>" name="emailx" placeholder="ex : email@gmail.com">
                                                        <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleFormControlSelect1">Jabatan</label>
                                                        <select class="form-control" id="exampleFormControlSelect1" name="jabatanx">
                                                            <option value="Kasir" <?php if ($data_karyawan['jabatan'] == 'Kasir') {
                                                                                        echo "selected";
                                                                                    } ?>>Kasir</option>
                                                            <option value="Customer Service" <?php if ($data_karyawan['jabatan'] == 'Customer Service') {
                                                                                                    echo "selected";
                                                                                                } ?>>Customer Service</option>
                                                            <option value="Supir" <?php if ($data_karyawan['jabatan'] == 'Supir') {
                                                                                        echo "selected";
                                                                                    } ?>>Supir</option>
                                                        </select>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="gaji">Gaji Karyawan (IDR)</label>
                                                        <div class="input-group-prepend">
                                                            <span class="input-group-text" id="basic-addon1">Rp</span>
                                                            <input type="number" class="form-control" id="gaji" value="<?= $data_karyawan['gaji']; ?>" required name="gajiKaryawan" placeholder="ex : 200000">
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleFormControlSelect2">Status Gaji</label>
                                                        <select class="form-control" id="exampleFormControlSelect2" name="statusGaji">
                                                            <option value="Selesai" <?php if ($data_karyawan['status_gaji'] == 'Selesai') {
                                                                                        echo "selected";
                                                                                    } ?>>Selesai</option>
                                                            <option value="Belum" <?php if ($data_karyawan['status_gaji'] == 'Belum') {
                                                                                        echo "selected";
                                                                                    } ?>>Belum</option>
                                                        </select>
                                                    </div>
